...
| **Gateway event hooks** (fire on `gateway:startup`, `session:start`, `agent:end`, `job:end`, `command:*`) | Drop `HOOK.yaml` + `handler.py` into `~/.hermes/hooks/<name>/` | [Event Hooks](/user-guide/features/hooks#gateway-event-hooks) |
